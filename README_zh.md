...
> [!IMPORTANT]
> **Google 日历兼容性**: Google 日历原生 **不支持** `VTODO`。如需在 Google 日历中显示仅日期任务，请在调度设置中启用 **"将日期任务显示为全天事件"**，或在每个任务中添加具体时间（如 `13:00`）。
...
答：Google 日历原生仅支持 `VEVENT`。在调度设置中启用 **”将日期任务显示为全天事件”** 即可将仅日期任务导出为全天事件。无日期的浮动任务仍为 `VTODO`，不会显示。如需完整的 `VTODO` 支持，建议使用 Apple Calendar、Microsoft Outlook 或专业任务管理应用。
...
- **Google 日历不支持 `VTODO`。** 默认情况下，仅日期任务导出为 `VTODO`。在调度设置中启用 **"将日期任务显示为全天事件"** 可将其导出为全天 `VEVENT`。此为可选设置，推荐 Google 日历用户使用。无日期的浮动任务不受此设置影响，仍为 `VTODO`。
- **日历客户端刷新订阅源可能较慢。** 大多数客户端会缓存订阅源，仅定期刷新（通常每 1–24 小时）。同步后可能需要手动刷新或等待下一次轮询周期。
- **本地文件导出需要支持读取本地 ICS 文件的日历客户端。** 并非所有客户端都支持订阅本地文件。Apple Calendar 和 Thunderbird 兼容性良好；部分客户端可能需要通过 HTTP 服务该文件。
- **移动端支持因客户端和存储工作流而异。** 如果使用本地文件导出，`.ics` 文件存储在 Obsidian 库中，移动端日历应用可能无法直接访问库存储。建议跨设备使用 GitHub Gist 同步。
